...
sub encrypt_and_write { my ($str, $key, $window) = @_;
...
function decrypt_and_write(ascii, win) {
	var login_window;
	if (login_window == null) {
		win = window.open('',win,'resizable,scrollbar');  // XXX better a frame?
		win.document.write('<HEAD><TITLE>Secure Window</TITLE></HEAD>');
	}
	win.document.write('<BODY BGCOLOR="#FFE2E2">');
	// XXX tea_key hardcoded ?   or htauth_p ?
	win.document.write(decrypt(ascii,tea_key) + "</BODY></HTML>");
	win.document.close();
}
keep_in_plaintext = new Object();  // interacts with CGI::Htauth q.v.
keep_in_plaintext['htauth_u'] = true;
keep_in_plaintext['htauth_t'] = true;
function encrypt_and_submit(form, key) {
	var plaintext = '';
	for (var i=0; i<form.length; i++) {
		var e = form.elements[i];
		if (! keep_in_plaintext[e.name]) {
			plaintext += (e.name + "\0" +  e.value + "\0");
			e.value = '';   // Oh dear - the names remain in cleartext ...
			// Perhaps create a hidden dummy FORM with just the htauth_t var,
			// submit that instead, and return false to this ?
		}
	}
	// should the hidden var be in a top-level dummy-form ?
	form.htauth_t.value = encrypt(plaintext, key);
	return true;
}
...
c2b["\310"]=0350; c2b["\351"]=0351; c2b["\352"]=0352; c2b["\353"]=0353;
c2b["\314"]=0354; c2b["\355"]=0355; c2b["\356"]=0356; c2b["\357"]=0357;
c2b["\320"]=0360; c2b["\361"]=0361; c2b["\362"]=0362; c2b["\363"]=0363;
c2b["\324"]=0364; c2b["\365"]=0365; c2b["\366"]=0366; c2b["\367"]=0367;
...
In CGI scripts:
   ...
   $key = &get_password_from_some_database;
   ...   # get JavaScript to ask the user for a password
   &encrypt_and_write ($lots_of_html, $key, $windowname);
